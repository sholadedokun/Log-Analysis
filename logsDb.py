...
def get_errorDayAbovePercentage(percentValue):
  ...
  db=psycopg2.connect(database=Dbase)
  cursor=db.cursor()
  cursor.execute("select to_char(day, 'FMMonth DD, YYYY'), concat(round(error_percent::DECIMAL, 2), '%%') as percent_error from (select a.day as day, ((b.error_count::float/a.totalHit::float)*100::float) as error_percent from LogsPerDay as a, errorsPerDay as b where a.day=b.day) as subQuery  where error_percent >%d order by error_percent desc;" %(percentValue))
